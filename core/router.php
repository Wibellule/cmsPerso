...
class Router
{
	...
	**/
	...
	static function webroot($url)//Pour avoir les chemins jusqu'à la racine
	{
		trim($url,'/');
		return BASE_URL.'/'.$url; 
	}
...
// {
	...
	***/
	// static function parse($url,$request)
	// {
		// $url = trim($url,'/');//Permet d'enlever les espaces au début et en fin de chaîne
		// if(empty($url))
		// {
			// $url = router::$routes[0]['url'];
		// }
		...
		// $request->controller = $params[0];
		// $request->action = isset($params[1]) ? $params[1] : 'index';
		// $request->params = array_slice($params,2);//Pour sauter des élements
		// return true;
	// }
